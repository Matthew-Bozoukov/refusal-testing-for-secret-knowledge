...
class ModelBase(ABC):
    ...
    def generate_completions(self, dataset, fwd_pre_hooks=[], fwd_hooks=[], batch_size=8, max_new_tokens=64, system=None, translation=False, prefill=None):
        ...
        for i in tqdm(range(0, len(dataset), batch_size)):
            # Prepare prefill outputs if provided
            outputs = None
            if prefill is not None:
                batch_size_actual = min(batch_size, len(dataset) - i)
                outputs = [prefill] * batch_size_actual

            tokenized_instructions = self.tokenize_instructions_fn(instructions=instructions[i:i + batch_size], outputs=outputs, system=system)
            ...
            with add_hooks(module_forward_pre_hooks=fwd_pre_hooks, module_forward_hooks=fwd_hooks):
                ...
                for generation_idx, generation in enumerate(generation_toks):
                    ...
                    # Only add instruction_en if dataset contains dicts
                    if not isinstance(dataset[0], str):
                        completion['instruction_en'] = instructions_en[i + generation_idx]
